...
| `ValidationError` | `validate.rs` | Why a graph cannot run | `enum ValidationError { TypeMismatch { edge: Edge, expected: PortType, found: PortType }, Cycle(Vec<NodeId>), MissingInput { node: NodeId, port: String }, TierUnavailable { node: NodeId, required: DeviceTier } }` |
...
| `SocProfile` | `capability.rs` | Probed device facts, cached after first launch | `struct SocProfile { soc_id: String, soc_name: String, tier: DeviceTier, ram_bytes: u64, backends: Vec<Backend>, npu_experimental: bool }` |
...
| `resolve_availability` | `availability.rs` | **The precedence rule lives here and nowhere else.** `TierLimited` is returned before any commercial state is considered | `fn resolve_availability(kind: NodeKind, caps: &SocProfile, ent: &Entitlement, credits: u32, model_present: bool) -> NodeAvailability` |
...
| `Scheduler::run` | `scheduler.rs` | Executes a plan, honouring the governor and emitting progress | `fn run(&mut self, plan: &JobPlan, sink: &mut dyn ProgressSink) -> Result<(), CoreError>` |
...
| Entity | Module | Role | Signature |
| --- | --- | --- | --- |
| `Engine` | `lib.rs` | Uniform inference interface | `trait Engine { fn load(&mut self, model: &ModelRef) -> Result<(), EngineError>; fn run(&mut self, input: TensorIo) -> Result<TensorIo, EngineError>; fn backend(&self) -> Backend; }` |
| `TensorIo` | `lib.rs` | Engine-agnostic tensor payload | `struct TensorIo { shape: Vec<usize>, dtype: DType, data: Vec<u8> }` |
